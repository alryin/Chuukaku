...
#include <algorithm> // Per poter utilizzare std::swap
...
class pair{
	...
	const K &key() const {					//metodo getter e setter sulla chiave (quello costante) solo in lettura
		return _key;
	}
	...
	const T &value() const {					//metodo getter e setter sul valore (quello costante) solo in lettura
		return _value;
	}
